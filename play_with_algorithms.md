...
# 树结构
本身是一种天然的组织结构。将数据使用树结构存储，出奇的高效，包括以下几种类型
* 平衡二叉树：AVL，红黑树
* 堆：并查集
* 线段树：主要处理线段
...
* 一种动态数据结构，和链表一样，
* 具有唯一的根节点，
* 最多有两个孩子，左孩子，右孩子，没有孩子的节点称为叶子节点
* 每个节点最多有一个父亲节点
* 具有天然递归结构：每一个节点的左孩子也是一个二叉树，成为左子数。每一个节点的右孩子也是一个二叉树，成为右子树
* 一个节点也可以是一个二叉树
* 一个NULL也可以是二叉树
...
#### 广度优先遍历
* 层序遍历 逐层向下 在广度上进行拓展，通常使用非递归，使用队列实现
...
### 删除左右都有孩子的节点
1. 找到该节点的后继结点s，也就是第一个比该节点大的值，右节点数中最小的那个  s=getMin(d.right)
...
## 映射 Map
或者称为字典，是一个内容和另外一个内容搭配来存储数据。存储键值对（key ,value）的数据结构。使用二分搜索树或者链表可以实现。例如
* 身份证--> 人
* 车牌号--> 车
* 单词统计  单词--->词频   
...
## 二叉堆：
是一个完全二叉树 ，不一定是一个满二叉树，但是缺失那一部分一定得在树的右侧
把元素顺序排列成树的形状
* 最大堆
某个节点的值总是不大于其父节点的值。可以使用数组的方式实现二叉堆
...
## 哈希表
类型转成索引的函数，就称之为哈希函数的索引相同
哈希冲突： 两个键通过哈希函数的转换
在哈希表中的操作，最复杂的部分就是解决哈希冲突
哈希表充分体现了算法领域中的经典思想：  空间换时间的思想
哈希表是时间和空间的之间平衡

键通过哈希函数得到的索引分布越均匀越好，

哈希函数设计：
对于大整型： 通常的一个做法就是取模，模一个素数
浮点型： 转成一个大整型，然后在取模，
字符串：如果只是小写的，可以转换成26进制，然后变成整型，然后再取模
![Alt text](https://github.com/hoyouly/BlogResource/raw/master/imges/string_2_hashcode.png)

```java
//B 是进制数，可以自定义，比如二十六进制，或者52进制等，M是取模素数
int hash=0;
for (int i = 0; i < s.length(); i++) {
    hash=(hash*B+s.charAt(i))%M
}
```
复合型: 可以看成字符串，先转成整型，然后在取模求余
...
集合和映射分两种
* 有序版，底层实现就是平衡树，有序集合，有序映射 TreeMap,TreeSet
* 无序版，底层实现是哈希表，无序集合，无序映射 HashMap,HashSet
